...
**DB 에는 409편이 다 있지만 앱은 한 번에 5편만 보여준다.** 생활회화는 `getVideos('daily')`
가 오늘부터 `TODAY_VIDEOS`(5)편만 돌려준다 — 홈 캐러셀과 같은 구간이다. 409편을 목록에
다 그리면 오래된 기기에서 버거운데다, 매일 한 편 보는 앱에 어울리지도 않는다.
지난 회차는 즐겨찾기(`getVideosByIds`)로만 다시 볼 수 있다.

커리큘럼이 2027-06-13 에 끝나면 어댑터가 `deriveVideosFromDay` 순환 폴백으로 떨어진다
