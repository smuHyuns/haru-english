...
describe('영상 화면', () => {
  it('전체 카테고리에서 영상 6개를 보여준다', async () => {
    renderScreen(<Videos />);
    await screen.findByText('아침에 쓰는 인사 표현 10가지');
  ...
  it('URL 의 cat 파라미터로 필터링한다', async () => {
    ...
    expect(screen.getByText('날씨 이야기로 대화 시작하기')).toBeInTheDocument();
    expect(screen.queryByText('공항에서 바로 쓰는 문장')).not.toBeInTheDocument();
  });
  ...
  it('칩을 누르면 필터가 바뀐다', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: '병원' }));
    await waitFor(() => expect(screen.getAllByRole('listitem')).toHaveLength(1));
    expect(screen.getByText('병원에서 증상 말하기')).toBeInTheDocument();
  });
  ...
  it('즐겨찾기 토글 시 토스트가 뜬다', async () => {
    ...
    await userEvent.click(
      screen.getByRole('button', { name: '아침에 쓰는 인사 표현 10가지 즐겨찾기' }),
    );
...
describe('즐겨찾기 화면', () => {
  ...
  it('영상 세그먼트로 전환하면 즐겨찾기한 영상을 보여준다', async () => {
    ...
    expect(await screen.findByText('공항에서 바로 쓰는 문장')).toBeInTheDocument();
    expect(screen.getAllByRole('listitem')).toHaveLength(1);
  });

  it('영상을 전부 지우면 영상 빈 상태가 나온다', async () => {
    renderScreen(<Saved />, '/saved?view=videos');
    await screen.findByText('공항에서 바로 쓰는 문장');

    await userEvent.click(screen.getByRole('button', { name: '공항에서 바로 쓰는 문장 즐겨찾기 해제' }));
