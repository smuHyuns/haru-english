...
describe('영상 화면', () => {
  ...
  it('영상 개수를 보여준다', async () => {
    ...
    await userEvent.click(screen.getByRole('radio', { name: '등록순' }));
    // 정렬을 바꿔도 개수는 그대로
    expect(screen.getByText('영상 7개')).toBeInTheDocument();
  });

  it('기본 정렬은 이름순이다', async () => {
    renderScreen(<Videos />, '/videos?cat=daily');
    await screen.findByText('영상 3개');
    expect(screen.getByRole('radio', { name: '이름순' })).toBeChecked();
    expect(screen.getByRole('radio', { name: '등록순' })).not.toBeChecked();

    // '10강' 이 '2강' 앞에 오면 안 된다 (numeric collation)
    const titles = screen.getAllByRole('listitem').map((li) => li.textContent ?? '');
    expect(titles[0]).toContain('1강');
    expect(titles[1]).toContain('2강');
    expect(titles[2]).toContain('3강');
  });

  it('등록순은 URL 에 남아 새로고침해도 유지된다', async () => {
    renderScreen(<Videos />, '/videos?cat=daily&sort=order');
    expect(screen.getByRole('radio', { name: '등록순' })).toBeChecked();
    // 카테고리 필터도 같이 살아 있어야 한다
    await waitFor(() => expect(screen.getAllByRole('listitem')).toHaveLength(3));
  });

  it('카테고리를 바꿔도 정렬 기준이 유지된다', async () => {
    renderScreen(<Videos />, '/videos?sort=order');
